...
function createMcpServer() {
  ...
  server.registerTool(
    ...
    {
      description:
        'Baca beberapa halaman sekaligus dari satu kitab (rentang halaman). ' +
        'Berguna untuk membaca pembahasan panjang atau bab yang mencakup banyak halaman. ' +
        'Maksimal 10 halaman per permintaan.',
      inputSchema: z.object({
        book_id: z.number().int().positive().describe(
        ...
        sampai_halaman: z.number().int().positive().describe(
          'Nomor halaman akhir (page_id). Maksimal dari_halaman + 9'
        ),
      }),
    },
    async ({ book_id, dari_halaman, sampai_halaman }) => {
      try {
        // Batasi maksimal 10 halaman
        const akhir = Math.min(sampai_halaman, dari_halaman + 9);
        const pageNums = Array.from(
          { length: akhir - dari_halaman + 1 },
          (_, i) => dari_halaman + i
        );

        const pages = await Promise.all(
          pageNums.map(pg =>
            getPage(book_id, pg).catch(e => ({ error: e.message, pg }))
          )
        );

        const parts = pages.map((p, i) => {
          if (p.error) return `[Hal ${pageNums[i]}] ❌ ${p.error}`;
          const m   = p.meta || {};
          const bab = (m.headings || []).filter(Boolean).join(' › ');
          return [
            `${'─'.repeat(50)}`,
            `📄 Hal: ${m.page} (page_id: ${m.page_id})` + (bab ? ` | 🔖 ${bab}` : ''),
            '',
            p.text,
          ].join('\n');
        });

        // Header dari halaman pertama yang berhasil
        const firstOk = pages.find(p => !p.error);
        const m = firstOk?.meta || {};
        const header = `📖 ${m.book_name || 'Kitab'} | ✍️ ${m.author_name || '—'}\n` +
                       `📚 Vol: ${m.vol || '—'} | Halaman ${dari_halaman}–${akhir}\n`;
